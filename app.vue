<template>
  <div class="p-5 h-[calc(100vh-40px)] flex">
    ...
    <div class="w-1/4 pr-5 h-full overflow-y-auto border-r">
      <div class="mb-5">
        <h3 class="font-bold">Filter by Options</h3>
      ...
      <div class="grid grid-cols-4 gap-5">
        <div v-for="car in filteredCars" :key="car.id" class="p-5 border">
          ...
          <div class="flex gap-2 items-center">
            <div class="font-bold">{{ formatCurrency(car.listPrice) }}</div>
            <div class="text-sm">{{ formatCurrency(car.catalogPrice) }}</div>
          </div>
          <div class="text-gray-600">{{ car.dealer }}</div>
...
<style scoped>
/* Add this for styling if needed */
</style>
