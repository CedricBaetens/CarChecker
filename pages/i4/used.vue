<template>
  <div class="flex">
    ...
    <div class="w-1/4 p-4 bg-gray-100">
      ...
      <!-- Prioritized Options -->
      <div v-for="option in prioSortedOptions" class="mb-2">
      ...
      <!-- Remaining Options -->
      <div v-for="option in remainingSortedOptions" class="mb-2">
        <label class="flex items-center">
          <input
            v-model="selectedOptions"
            type="checkbox"
...
// Fetch data from API
const { data } = useFetch('/api/i4/used')
...
const prioSortedOptions = computed(() => {
  const result = _.intersection(prioOptions, Array.from(optionsSet.value))
  return result.sort()
})

const remainingSortedOptions = computed(() => {
  const result = _.difference(Array.from(optionsSet.value), prioOptions)
  return result.sort()
})
